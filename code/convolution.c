...
{
    int i, j, m, n, chunkY, chunkX, kernelY;
    ...
    chunkY = dataSizeY / omp_get_max_threads();
    chunkX = dataSizeX / omp_get_max_threads();
        ...
        while (c < partitions) {
            ...
            #pragma omp parallel
            {
                #pragma omp sections nowait
                {
                    ...
                    #pragma omp section
                    convolve2D(source->G, output->G, source->ancho, (source->altura / partitions) + halosize,
                               kern->vkern,
                               kern->kernelX, kern->kernelY);
                    #pragma omp section
                    convolve2D(source->B, output->B, source->ancho, (source->altura / partitions) + halosize,
                               kern->vkern,
                               kern->kernelX, kern->kernelY);
                }
            }
